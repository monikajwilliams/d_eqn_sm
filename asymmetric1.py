...
def u0gauss(
    ...
    val = np.exp(-1000.0 * ((x+3.0)**2 + (y+3.0)**2 + (z+0.0)**2))
    r = np.sqrt(x**2 + y**2 + z**2)
    val[np.where(r<Ri)] = 0.0
...
def make_gifs(
    ...
    os.system('mkdir scrap')

    print 'collocating initial conditions'
    v0 = sphere.colloc(0,sphere.xyzw[:,0],sphere.xyzw[:,1],sphere.xyzw[:,2])
    w = sphere.xyzw[:,3]
    norm = np.sum(v0*w)
    top = np.max(v0/norm)+0.1
    bot = np.min(v0/norm)-0.1

    if log == False:
        lvls = np.linspace(bot,top,500)
        interval = np.arange(0,len(lvls),50)  
        labels = []
        for label in lvls[interval]:
            labels.append('%1.2f' % (label))
    else:
    ...
    for t in t_plots:
        ...
        plt.savefig('scrap/%07.1f_k.png'%(t))
        plt.close()
    os.system('/usr/local/bin/convert -delay 20 -loop 0 "scrap/*.png" %s' % (filename))
